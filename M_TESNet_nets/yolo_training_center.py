...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        if self.loss_type == "iou":
            ...
            v = (4 / (torch.pi ** 2)) * torch.pow((torch.atan(
                pred[:, 2] / torch.clamp(pred[:, 3], min=1e-6)) - torch.atan(
                target[:, 2] / torch.clamp(target[:, 3], min=1e-6))), 2)
            ...
            # 计算对角线距离
            enclose_diagonal = torch.sum(torch.pow(enclose_wh, 2), axis=-1)
            # EIou公式：基于IoU和中心点、包围框的对角线距离
            eiou = iou - (center_distance / enclose_diagonal).clamp(min=1e-6)
            loss = 1 - eiou
        elif self.loss_type == 'niou':
            eps = 1e-7
            constant = 12.8
            iou_ratio = 0
            # 计算相似度
            ...
            b1_wh = pred[:, 2:]
            b2_wh = target[:, 2:]
            center_distance = torch.sum(torch.pow((b1_cxy - b2_cxy), 2), axis=-1) + eps
            wh_distance = torch.sum(torch.pow((b1_wh - b2_wh), 2), axis=1) / 4 + eps
            nwd = torch.exp(-torch.sqrt(center_distance + wh_distance) / constant)
            loss = (1 - iou_ratio) * (1 - nwd) + iou_ratio * (1 - iou ** 2)
...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, fp16, strides=[8, 16, 32], weight_decay=0.05):
        super().__init__()
    ...
    def forward(self, inputs, labels=None, model=None):
        outputs = []
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        ...
        losses = self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1))
        ...
        if model is not None:
            ...
            losses += self.weight_decay * l2_reg  # Adding L2 regularization term
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        # -----------------------------------------------#
        bbox_preds = outputs[:, :, :4]  # cx, cy, w, h (one anchor)
        # -----------------------------------------------#
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                # TODO ----------------- 中心点 Smooth L1 -----------------#
                if fg_mask.sum() > 0:
                    pred_centers = bboxes_preds_per_image[fg_mask, :2]
                    gt_centers = reg_target[:, :2]
                    # 可以对小目标加权
                    small_mask = (reg_target[:, 2] * reg_target[:, 3] <= 9).float()  # 3x3小目标
                    weight = 1.0 + small_mask  # 小目标权重加倍
                    loss_center = F.smooth_l1_loss(pred_centers, gt_centers, reduction='none')
                    loss_center = (loss_center.sum(dim=-1) * weight).sum()
                    loss_center_total += loss_center
        ...
        reg_weight = 5.0
        # TODO 中心点损失
        loss = reg_weight * loss_iou + loss_obj + 0.1 * loss_cls + 1.0 * loss_center_total
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
        ...
        # TODO 新增中心距离成本----------------#
        pred_centers = bboxes_preds_per_image[:, :2].unsqueeze(0).repeat(num_gt, 1, 1)
        gt_centers = gt_bboxes_per_image[:, :2].unsqueeze(1).repeat(1, pred_centers.shape[1], 1)
        center_distance = torch.sum((pred_centers - gt_centers) ** 2, dim=-1)
        cost += 10.0 * center_distance  # 可调整权重
        # TODO -----------------------------#
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        for gt_idx in range(num_gt):
            ...
            k_val = dynamic_ks[gt_idx].item()
            cost_len = cost[gt_idx].numel()
            assert k_val > 0 and k_val <= cost_len, f"Invalid k={k_val}, while cost length={cost_len}"
