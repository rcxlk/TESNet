...
from M_TESNet_nets.TESNet_3DSTF_ablation import TESNet
...
def normalize_per_channel(img):
    img = img.astype(np.float32)  # 先确保是 float 类型
    img_norm = np.zeros_like(img)

    for c in range(3):  # 对每个通道分别归一化
        channel = img[:, :, c]
        min_val = channel.min()
        ptp_val = channel.ptp()  # max - min

        if ptp_val > 1e-6:  # 避免除零
            img_norm[:, :, c] = (channel - min_val) / ptp_val * 255
        else:
            img_norm[:, :, c] = 0  # 如果该通道没有变化，就设为 0
...
class Pred_vid(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'logs_better_spatial2/loss_2025_04_28_18_39_08/last_epoch_weights.pth',
    ...
    def generate(self, onnx=False):
        self.net    = TESNet(self.num_classes, num_frame=num_frame)
...
if __name__ == "__main__":
    ...
    if mode == "predict":
        ...
        img = '/home/public/IRDST/68/100.jpg'
        try:
        ...
        # dir_path = r'E:/code/rcx_code/earth_moon_StarSimulation/DYPD_simulation/real_data/detect/'
        dir_path = r'E:\code\rcx_code\target_detection\SIMDataSet1\sim_dataset\images\train_sequences\no_light_snr_0.9/'
        # dir_path = r'E:\code\rcx_code\target_detection\My_Data_Tycho2_blur\sim_dataset\images\val_sequences\uniform_snr_1.1/'
        result = 'result_1'
        if not os.path.exists(result):
            os.makedirs(result)
        images = natsorted(os.listdir(dir_path))
        ...
        for image in tqdm(images):
            ...
            cv2.imwrite(os.path.join(result, '{}.png'.format(idx)), np.asarray(r_image))
            idx += 1
        ...
        outfile = cv2.VideoWriter("./output.avi", fourcc, 1, (256, 256), True)
        for i in list_img: 
